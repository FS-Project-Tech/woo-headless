...
export default function ProductsDisplay({ categorySlug }: ProductsDisplayProps) {
  ...
  if (error && products.length === 0) {
    return (
      <div className="rounded-lg bg-white p-8 text-center">
        ...
        <p className="text-sm text-gray-500">Please check your WooCommerce API configuration.</p>
      </div>
    );
  }
    ...
    <div className="relative min-h-[400px]">
      ...
      {hasMore && (
        <div ref={observerTarget} className="h-20 flex items-center justify-center">
          {loading && (
            <div className="inline-block h-6 w-6 animate-spin rounded-full border-3 border-gray-300 border-t-gray-900"></div>
          )}
        </div>
      )}
