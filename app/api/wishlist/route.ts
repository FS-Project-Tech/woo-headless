import { NextResponse } from 'next/server';
import { getAuthToken } from '@/lib/auth';

export async function GET() {
  try {
    const token = getAuthToken();
    if (!token) {
      return NextResponse.json({ wishlist: [] });
    }

    // Get WordPress base URL
    const api = process.env.NEXT_PUBLIC_WC_API_URL || '';
    const u = new URL(api);
    const wpBase = `${u.protocol}//${u.host}`;

    // Get current user
    const userRes = await fetch(`${wpBase}/wp-json/wp/v2/users/me`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!userRes.ok) {
      return NextResponse.json({ wishlist: [] });
    }
    // Get wishlist from custom WordPress endpoint
    const wishlistRes = await fetch(`${wpBase}/wp-json/custom/v1/wishlist`, {
      headers: { Authorization: `Bearer ${token}` },
    });

    if (!wishlistRes.ok) {
      return NextResponse.json({ wishlist: [] });
    }

    const result = await wishlistRes.json();
    return NextResponse.json({ wishlist: result.wishlist || [] });
  } catch (e: any) {
    return NextResponse.json({ wishlist: [] });
  }
}

