...
export async function POST(req: Request) {
  try {
    ...
    // Call custom WordPress endpoint to update user meta
    // This endpoint should be created via a WordPress plugin (see wordpress/wishlist-api.php)
    const updateRes = await fetch(`${wpBase}/wp-json/custom/v1/wishlist/add`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json({ error: e?.message || 'Failed to add to wishlist' }, { status: 500 });
  }
}
